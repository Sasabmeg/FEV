...
//								frameSelector.getCurrentFrameIndex(), file.toString(), frameSelector.isHasBackground());
	...
	private void setDefaultFileList() {
		...
		//Will set default directory to the directory from where the program will run.
		fileSelector.setCurrentFolder(System.getProperty("user.dir"));
		fileSelector.setCurrentExportFolder(System.getProperty("user.dir"));
		/*
		...
		scene.setOnKeyPressed(new EventHandler<KeyEvent>() {
			public void handle(final KeyEvent keyEvent) {
				if (keyEvent.getCode() == KeyCode.PAGE_UP) {
					fileSelector.prevSkin();
					showCurrentFrame(true);
					keyEvent.consume();
				}
				if (keyEvent.getCode() == KeyCode.PAGE_DOWN) {
					fileSelector.nextSkin();
					showCurrentFrame(true);
					keyEvent.consume();
				}
